...
import { registraNovoUsuario } from "../services/registraNovoUsuario";
...
export default function Cadastro() {
  ...
  function handleChangeCaptureDataUser(event) {
    const { name, value } = event.target;

    setDataUser((prevDataUser) => ({ ...prevDataUser, [name]: value }));
  }

  async function handleSubmitAddNewUser(event) {
    event.preventDefault();

    const data = {
      ...dataUser,
      telefone: Number(dataUser.telefone),
    };

    setLoading(true);

    const response = await registraNovoUsuario(data);
    ...
    <main className="w-full flex">
      ...
      <div className="flex-1 flex items-center justify-center h-screen">
        <div className="w-full max-w-md space-y-8 px-4 bg-white text-gray-600 sm:px-0">
          ...
          <form onSubmit={handleSubmitAddNewUser} className="space-y-5">
            <div>
              <label className="font-medium">Nome</label>
              <input
                type="text"
                name="nome"
                onChange={handleChangeCaptureDataUser}
                value={dataUser.nome}
                required
